...
constexpr char kQueryCountTag[] = "inspire/query_count";
...
std::string QueryTag(uint64_t idx) {
  return fmt::format("inspire/query/{}", idx);
}

std::string ResponseTag(uint64_t idx) {
  return fmt::format("inspire/response/{}", idx);
}

InspireParameters BuildParameters(uint64_t db_rows, uint64_t db_cols,
                                  uint64_t item_size_bits) {
  YACL_ENFORCE_GT(db_rows, 0U);
  YACL_ENFORCE_GT(db_cols, 0U);
  YACL_ENFORCE_EQ(item_size_bits, 8U,
                  "Inspire launcher currently supports 8-bit values");
  return CreateParamsForShape(db_rows, db_cols, item_size_bits);
}
...
int SenderOnline(const InspireSenderOptions& options,
  ...
  YACL_ENFORCE(lctx != nullptr, "link context is required for Inspire sender");
  const auto params =
      BuildParameters(options.db_rows, options.db_cols, options.item_size_bits);

  SPDLOG_INFO("Starting Inspire sender, db_rows={}, db_cols={}", params.db_rows,
              params.db_cols);

  InspireServer server(params);
...
int ReceiverOnline(const InspireReceiverOptions& options,
                   std::shared_ptr<yacl::link::Context> lctx) {
  YACL_ENFORCE(lctx != nullptr, "link context is required for Inspire receiver");
  const auto params =
      BuildParameters(options.db_rows, options.db_cols, options.item_size_bits);
  const auto query_indices =
      LoadQueryIndices(options.query_file, params.NumItems());

  SPDLOG_INFO("Starting Inspire receiver, query_count={}",
              query_indices.size());

  InspireClient client(params);
  lctx->ConnectToMesh();
